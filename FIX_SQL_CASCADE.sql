-- ============================================================
-- FIX: ORDERS + LOTTERY SYSTEM (With CASCADE fixes)
-- ============================================================

-- ============================================================
-- STEP 1: ลบ triggers ที่ depend on functions ก่อน
-- ============================================================
DROP TRIGGER IF EXISTS tr_create_lotto_ticket ON orders;
DROP TRIGGER IF EXISTS tr_check_lottery_results ON lotto_results;

-- ============================================================
-- STEP 2: ลบ functions (ตอนนี้ไม่มี dependencies แล้ว)
-- ============================================================
DROP FUNCTION IF EXISTS create_lotto_ticket_for_order();
DROP FUNCTION IF EXISTS generate_queue_number(TEXT, BOOLEAN);
DROP FUNCTION IF EXISTS get_queue_status(BIGINT);
DROP FUNCTION IF EXISTS sync_guest_to_member(UUID, UUID);
DROP FUNCTION IF EXISTS claim_guest_order(INT, TEXT);
DROP FUNCTION IF EXISTS check_lottery_results();
DROP FUNCTION IF EXISTS generate_lotto_number(INT);

-- ============================================================
-- STEP 3: สร้างตาราง (ถ้ายังไม่มี)
-- ============================================================

-- 3.1 Guest Identities
...
-- ============================================================
-- STEP 4: เพิ่ม columns ให้ orders
-- ============================================================
ALTER TABLE orders
...
-- 6.1 Generate Queue Number (FIXED: ใช้ชื่อตัวแปรไม่ซ้ำ column)
...
-- 6.3 Generate Lotto Number
CREATE OR REPLACE FUNCTION generate_lotto_number(p_length INT DEFAULT 6)
RETURNS TEXT
LANGUAGE plpgsql
AS $$
DECLARE
...
-- 6.4 Create Lotto Ticket Trigger
...
BEGIN
    ...
    v_draw_date := CASE 
        ...
        ELSE
            DATE_TRUNC('month', CURRENT_DATE)::DATE + INTERVAL '15 days'
    END;
    
    IF NEW.queue_display IS NOT NULL THEN
        v_ticket_number := RIGHT(LPAD(REPLACE(NEW.queue_display, 'A', '1'), 6, '0'), 6);
    ELSE
        v_ticket_number := generate_lotto_number(6);
    END IF;
...
BEGIN
    ...
    IF v_points_earned > 0 THEN
        ...
        UPDATE profiles
        SET points = v_current_points + v_points_earned,
        ...
        VALUES (
            ...
            'Points from ' || v_orders_count || ' guest orders (sync)',
...
BEGIN
    ...
    RETURN json_build_object('success', TRUE, 'points_earned', v_points, 'new_balance', v_points);
END;
$$;

-- 6.7 Check Lottery Results
...
BEGIN
    UPDATE lotto_tickets
    SET status = 'won', prize_type = 'free_meal', prize_amount = 0, updated_at = NOW()
    WHERE draw_date = NEW.draw_date
      AND status = 'active'
      AND (
          number = NEW.first_prize
          OR RIGHT(number, 2) = NEW.last2
          OR (LEFT(number, 3) = ANY(NEW.first3))
          OR (RIGHT(number, 3) = ANY(COALESCE(NEW.last3_digits, ARRAY[]::TEXT[])))
      );
    RETURN NEW;
END;
$$;
...
CREATE POLICY "guest_identities_insert" ON guest_identities FOR INSERT WITH CHECK (true);
CREATE POLICY "guest_identities_select" ON guest_identities FOR SELECT USING (true);
...
CREATE POLICY "lotto_tickets_select_own" ON lotto_tickets
    FOR SELECT USING (
        user_id = auth.uid() 
        OR guest_id::TEXT = COALESCE(current_setting('app.guest_id', true), '')
        OR EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND is_admin = true)
    );
CREATE POLICY "lotto_tickets_insert" ON lotto_tickets
    FOR INSERT WITH CHECK (user_id = auth.uid() OR EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND is_admin = true));

-- ============================================================
-- STEP 8: GRANTS
-- ============================================================
GRANT EXECUTE ON FUNCTION generate_queue_number(TEXT, BOOLEAN) TO authenticated, anon;
GRANT EXECUTE ON FUNCTION get_queue_status(BIGINT) TO authenticated, anon;
GRANT EXECUTE ON FUNCTION sync_guest_to_member(UUID, UUID) TO authenticated;
GRANT EXECUTE ON FUNCTION claim_guest_order(INT, TEXT) TO authenticated;
GRANT EXECUTE ON FUNCTION generate_lotto_number(INT) TO authenticated, anon;
...
SELECT 
    '✅ All Fixed!' as status,
    (SELECT COUNT(*) FROM information_schema.columns WHERE table_name = 'orders' AND column_name = 'guest_id') as has_guest_id,
    (SELECT COUNT(*) FROM information_schema.tables WHERE table_name = 'guest_identities') as has_guest_table,
    (SELECT COUNT(*) FROM information_schema.tables WHERE table_name = 'lotto_tickets') as has_tickets_table,
    (SELECT COUNT(*) FROM information_schema.routines WHERE routine_name = 'sync_guest_to_member') as has_sync,
    (SELECT COUNT(*) FROM information_schema.routines WHERE routine_name = 'generate_queue_number') as has_queue,
    (SELECT COUNT(*) FROM information_schema.triggers WHERE trigger_name = 'tr_create_lotto_ticket') as has_lotto_trigger;
