  ...
  return (
    <button
      ...
      className={cn(
        ...
        isSelected ? 'border-[#FF6B00] bg-[var(--bg-card)] shadow-xl shadow-orange-500/10' : 'border-white/10 bg-[var(--bg-card)] hover:border-white/10'
      )}
    >
      <div className={cn("w-14 h-14 rounded-2xl flex items-center justify-center transition-colors relative z-10", isSelected ? "bg-[#FF6B00] text-white shadow-inner" : "bg-[var(--bg-surface)] text-gray-400 group-hover:bg-gray-100 group-hover:text-gray-600")}>
        <Icon className="w-6 h-6" />
      </div>
      <div className="text-left flex-1 relative z-10">
        <p className={cn("font-black text-[17px] leading-tight", isSelected ? "text-white" : "text-gray-200")}>{title}</p>
...
function OrderSuccess({ order, onContinue }: { order: OrderWithLotto; onContinue: () => void }) {
  ...
  return (
    <div className="min-h-screen safe-area-pt flex flex-col items-center p-4 pb-32 overflow-y-auto relative" style={{ background: 'var(--bg-base)' }}>
      ...
      <div className="absolute top-[-10%] right-[-10%] w-[500px] h-[500px] bg-[var(--bg-card)]/20 rounded-full blur-[100px] pointer-events-none mix-blend-overlay" />
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ type: "spring", stiffness: 300, damping: 25 }} className="w-full max-w-md w-full relative z-10 pt-10">
        ...
        <div className="text-center mb-8">
           <motion.div initial={{ scale: 0, rotate: -20 }} animate={{ scale: 1, rotate: 0 }} transition={{ type: "spring", stiffness: 200, damping: 15, delay: 0.2 }} className="w-32 h-32 bg-[var(--bg-card)] rounded-[36px] flex items-center justify-center mx-auto mb-6 shadow-2xl shadow-green-900/20 border-4 border-white">
        ...
        <div className="space-y-6">
          {(order.paymentMethod === 'promptpay' || order.paymentMethod === 'transfer') && (
            <div className="bg-[var(--bg-card)] rounded-[40px] p-8 shadow-2xl relative overflow-hidden text-center border border-white">
               ...
               <h3 className="font-black text-xl text-white mb-1 relative z-10">ชำระเงิน</h3>
               <p className="text-sm font-bold text-gray-400 uppercase tracking-widest bg-[var(--bg-surface)] px-3 py-1 rounded-lg w-fit mx-auto mb-6 relative z-10">ยอดโอน {formatPrice(order.totalPrice)}</p>

               <div className="bg-[#FAFAF9] rounded-[32px] p-6 mb-6 shadow-md border border-white/10/50 relative z-10 group">
                  <div className="w-48 h-48 bg-[var(--bg-card)] rounded-[24px] shadow-sm flex items-center justify-center mx-auto mb-4 border border-white/10 overflow-hidden relative">
                  ...
                  <p className="font-black text-xl text-gray-200 tracking-tight">0XX-XXX-XXXX</p>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}>
            {order.lottoNumber ? (
               <div className="bg-gradient-to-br from-[#1E1B4B] via-[#4C1D95] to-[#312E81] rounded-[32px] p-8 shadow-2xl relative overflow-hidden text-center mb-6 border border-indigo-400/20">
                  <div className="absolute top-0 right-0 w-40 h-40 bg-[var(--bg-card)]/10 rounded-full blur-3xl pointer-events-none" />
                  ...
                  {order.lottoFortune && (
                    <div className="bg-[var(--bg-card)]/5 backdrop-blur-md rounded-2xl p-4 relative z-10 border border-white/10">
          ...
          <div className="bg-[var(--bg-card)] rounded-[32px] p-6 shadow-sm border border-white/10">
             <div className="flex items-center gap-3 mb-6 pb-4 border-b border-white/10/50">
                <div className="w-10 h-10 bg-[var(--bg-surface)] rounded-[14px] flex items-center justify-center"><Banknote className="w-5 h-5 text-gray-500" /></div>
                <div>
                   <h3 className="font-black text-white">สรุปคำสั่งซื้อ</h3>
             ...
             <div className="space-y-4">
                ...
                <div className="flex justify-between items-center">
                   ...
                   <span className="font-black text-white">{order.paymentMethod === 'cod' ? 'เงินสดจ่ายปลายทาง' : order.paymentMethod === 'transfer' ? 'โอนเงิน' : 'พร้อมเพย์'}</span>
                </div>
          ...
          <div className="flex flex-col gap-3 pt-4">
             <button onClick={() => { hapticLight(); navigate(`/orders/${order.id}${order.trackingToken ? `?token=${order.trackingToken}` : ''}`); }} className="h-16 bg-[var(--bg-card)] border border-white/10 text-white font-black text-base rounded-[24px] shadow-sm flex items-center justify-center gap-2 active:scale-95 transition-transform hover:bg-[var(--bg-surface)]">
               ดูรายละเอียดออเดอร์
             </button>
             <button onClick={() => { hapticLight(); onContinue(); }} className="h-14 font-black text-gray-500 text-sm active:scale-95 transition-transform hover:text-gray-200">
      ...
      <Container className="py-4 relative z-10 max-w-2xl mx-auto space-y-6">
        <div className="flex items-center justify-between sticky top-4 z-50">
          ...
          <div className="text-center">
            <h1 className="text-xl font-black text-white tracking-tight leading-none bg-clip-text text-transparent bg-gradient-to-r from-[#FF6B00] to-orange-400">ชำระเงิน</h1>
        ...
        <motion.div variants={staggerList} initial="hidden" animate="visible" className="space-y-6">
          ...
          <motion.div variants={slideUpItem} className="bg-[var(--bg-card)] backdrop-blur-xl rounded-[32px] p-2 shadow-sm border border-white">
             <div className="px-6 py-5 border-b border-white/10/50 flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-blue-50 flex items-center justify-center"><User className="w-5 h-5 text-blue-500"/></div>
                <h2 className="font-black text-gray-200">ข้อมูลผู้สั่งซื้อ</h2>
             </div>
             <div className="p-6 space-y-5">
                <div>
                   ...
                   <input type="text" value={customerName} onChange={(e) => setCustomerName(e.target.value)} placeholder={user?.displayName ? user.displayName : 'ระบุชื่อของคุณ เช่น สมชาย'} readOnly={deliveryMethod === 'workplace' && !!user?.displayName} className={cn("w-full h-14 px-5 rounded-[20px] border-2 font-bold focus:border-blue-500 outline-none transition-all", (deliveryMethod === 'workplace' && !!user?.displayName) ? "bg-gray-100/50 text-gray-400 border-transparent cursor-not-allowed" : "bg-[#FAFAF9] border-white/10 focus:bg-[var(--bg-card)] focus:shadow-sm")} />
                </div>
                <AnimatePresence>
                  {(deliveryMethod === 'village' || (deliveryMethod === 'workplace' && isGuest)) && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}>
                      ...
                      <input type="tel" value={phoneNumber} onChange={(e) => setPhoneNumber(e.target.value)} placeholder="เช่น 0812345678" className="w-full h-14 px-5 rounded-[20px] border-2 font-bold bg-[#FAFAF9] border-white/10 focus:bg-[var(--bg-card)] focus:border-blue-500 focus:shadow-sm outline-none transition-all" />
                    </motion.div>
                  )}
                </AnimatePresence>
             </div>
          ...
          <motion.div variants={slideUpItem} className="bg-[var(--bg-card)] backdrop-blur-xl rounded-[32px] p-2 shadow-sm border border-white">
             <div className="px-6 py-5 border-b border-white/10/50 flex items-center gap-3">
                ...
                <h2 className="font-black text-gray-200">วิธีรับอาหาร</h2>
             </div>
             <div className="p-4 space-y-4">
                <div className="flex bg-[var(--bg-base)] p-1.5 rounded-[24px] relative">
                   <button onClick={() => { hapticMedium(); setDeliveryMethod('workplace') }} className={cn("flex-1 h-14 rounded-[20px] font-black text-sm relative z-10 transition-colors duration-300 flex items-center justify-center gap-2", deliveryMethod === 'workplace' ? "text-white" : "text-gray-400")}>
                      <MapPin className="w-5 h-5"/> ที่ทำงาน
                   </button>
                   <button onClick={() => { hapticMedium(); setDeliveryMethod('village') }} className={cn("flex-1 h-14 rounded-[20px] font-black text-sm relative z-10 transition-colors duration-300 flex items-center justify-center gap-2", deliveryMethod === 'village' ? "text-white" : "text-gray-400")}>
                      <Truck className="w-5 h-5"/> หมู่บ้าน
                   </button>
                   <motion.div initial={false} animate={{ x: deliveryMethod === 'workplace' ? '0%' : '100%' }} className="absolute top-1.5 bottom-1.5 w-[calc(50%-6px)] bg-[var(--bg-card)] rounded-[20px] shadow-sm border border-black/5 z-0" transition={{ type: "spring", stiffness: 450, damping: 30 }} />
                </div>
                ...
                <AnimatePresence>
                  {deliveryMethod === 'village' && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="pt-2 pb-2 px-2">
                       ...
                       <textarea value={address} onChange={(e) => setAddress(e.target.value)} placeholder="ระบุบ้านเลขที่, ซอย (อย่างน้อยต้องมีเพื่อจัดส่งได้อย่างแม่นยำ)" rows={3} className="w-full p-5 rounded-[24px] border-2 font-bold bg-[#FAFAF9] border-white/10 focus:bg-[var(--bg-card)] focus:border-[#FF6B00] focus:shadow-sm outline-none transition-all resize-none shadow-inner" />
                    </motion.div>
                  )}
                </AnimatePresence>
             </div>
          ...
          <motion.div variants={slideUpItem} className="bg-[var(--bg-card)] backdrop-blur-xl rounded-[32px] p-2 shadow-sm border border-white">
             <div className="px-6 py-5 border-b border-white/10/50 flex items-center gap-3">
                ...
                <h2 className="font-black text-gray-200">วิธีชำระเงิน</h2>
             </div>
          ...
          <motion.div variants={slideUpItem} className="grid grid-cols-1 md:grid-cols-2 gap-6">
             <div className="bg-[var(--bg-card)] backdrop-blur-xl rounded-[32px] p-6 shadow-sm border border-white">
                <h2 className="font-black text-gray-200 mb-4 text-sm uppercase tracking-widest">หมายเหตุพิเศษ</h2>
                <textarea value={specialInstructions} onChange={(e) => setSpecialInstructions(e.target.value)} placeholder="ไม่ใส่ผัก เผ็ดน้อย พิเศษข้าว..." rows={2} className="w-full p-4 rounded-[20px] border-2 font-bold bg-[#FAFAF9] border-white/10 focus:bg-[var(--bg-card)] focus:border-gray-300 outline-none transition-all resize-none" />
             </div>
             <div className="bg-[var(--bg-card)] backdrop-blur-xl rounded-[32px] p-6 shadow-sm border border-white flex flex-col justify-center">
          ...
          {!isGuest && user && (
            <motion.div variants={slideUpItem} className="bg-gradient-to-br from-[#1C1917] to-[#292524] rounded-[32px] p-6 shadow-xl relative overflow-hidden">
               ...
               <div className="flex gap-4 relative z-10">
                  <div className="bg-[var(--bg-card)]/10 backdrop-blur-md rounded-2xl p-4 flex-1 border border-white/10">
          ...
          {/* Summary Details */}
          <motion.div variants={slideUpItem} className="bg-[var(--bg-card)] backdrop-blur-xl rounded-[32px] p-6 shadow-sm border border-white mb-24">
             <h2 className="font-black text-gray-200 mb-6 pb-4 border-b border-white/10/50">สรุปยอดที่ต้องชำระ</h2>
             <div className="space-y-4">
                <div className="flex justify-between items-center bg-[#FAFAF9] p-4 rounded-2xl border border-white/10/50">
                  <span className="text-sm font-bold text-gray-500">ยอดรวม ({items.reduce((sum, i) => sum + i.quantity, 0)} รายการ)</span>
                  <span className="font-black text-gray-200">{formatPrice(subtotal)}</span>
                </div>
           ...
           <motion.button whileTap={{ scale: 0.96 }} disabled={isProcessing} onClick={handlePlaceOrder} className="h-16 px-10 bg-[#FF6B00] text-white rounded-[24px] shadow-xl shadow-[#FF6B00]/40 flex items-center gap-3 relative overflow-hidden group border border-[#FF6B00] disabled:opacity-50 disabled:grayscale transition-all flex-shrink-0">
             ...
             {isProcessing ? (
               ...
               <>
                  ...
                  <div className="w-8 h-8 rounded-xl bg-[var(--bg-card)]/20 flex items-center justify-center relative z-10 backdrop-blur-md">
